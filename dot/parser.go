...
// It tokenizes the input, builds the AST, and assigns stable edge IDs before returning.
...
func (p *parser) parseStatement() error {
	...
	case TokenNode:
		// node [defaults]
		return p.parseNodeDefaultsStmt()

	case TokenEdge:
		// edge [defaults]
		return p.parseEdgeDefaultsStmt()
...
// parseNodeDefaultsStmt parses: 'node' AttrBlock ';'?
func (p *parser) parseNodeDefaultsStmt() error {
...
// parseEdgeDefaultsStmt parses: 'edge' AttrBlock ';'?
func (p *parser) parseEdgeDefaultsStmt() error {
...
func (p *parser) parseSubgraph() error {
	...
	for p.current().Type != TokenRBrace && p.current().Type != TokenEOF {
		...
		case TokenNode:
			...
			p.skipSemicolon()
		case TokenEdge:
			if err := p.parseEdgeDefaultsStmt(); err != nil {
				return err
			}
		case TokenGraph:
...
// parseValue parses a value: String | Number | Boolean | Identifier.
