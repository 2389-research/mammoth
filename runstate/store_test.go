...
func newTestRunState(t *testing.T) *RunState {
	...
	return &RunState{
		...
		Context:        map[string]string{"model": "gpt-4"},
		Events:         []RunEvent{},
	}
}
...
func TestRunStateGet(t *testing.T) {
	store := newTestStore(t)
	state := newTestRunState(t)
	state.Context = map[string]string{"model": "gpt-4", "temperature": "0.7"}
...
func TestRunStateAddEvent(t *testing.T) {
	...
	if got.Events[0].Type != "pipeline.started" {
		t.Errorf("event 0 type mismatch: got %q, want %q", got.Events[0].Type, "pipeline.started")
	}
	if got.Events[1].Type != "stage.started" {
		t.Errorf("event 1 type mismatch: got %q, want %q", got.Events[1].Type, "stage.started")
	}
...
func TestRunStateAddEventNonexistent(t *testing.T) {
	...
	err := store.AddEvent("nonexistent-id", RunEvent{Type: "pipeline.started"})
	if err == nil {
...
func TestRunStateCorruptEventsJSONL(t *testing.T) {
	...
	if err := store.AddEvent(state.ID, RunEvent{Type: "pipeline.started"}); err != nil {
...
// --- SourceHash function tests ---

func TestSourceHash(t *testing.T) {
	hash1 := SourceHash("digraph test { a -> b }")
	hash2 := SourceHash("digraph test { a -> b }")
	hash3 := SourceHash("digraph test { a -> c }")
	...
	if len(hash1) != 64 {
		t.Errorf("expected 64-char hex string (SHA-256), got %d chars", len(hash1))
	}
}
